...
public class SimpleModel {
    ...
    protected void renderQuads(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int order,
                               RenderType renderType, List<SimpleQuad> quads,
                               int color, int lightmapCoord, int overlayColor) {
        float alpha = (float)(color >>> 24 & 255) / 255.0F;
        float red   = (float)(color >>> 16 & 255) / 255.0F;
        float green = (float)(color >>> 8 & 255) / 255.0F;
        float blue  = (float)(color & 255) / 255.0F;
        renderQuads(poseStack, submitNodeCollector, order, renderType, quads, lightmapCoord, overlayColor,
                red, green, blue, alpha);
    }

    protected void renderQuads(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int order,
                               RenderType renderType, List<SimpleQuad> quads, int lightmapCoord, int overlayColor,
