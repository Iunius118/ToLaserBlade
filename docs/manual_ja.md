...
3. 「LB電池」「レーザーブレイド放射器」「レーザーブレイドの外装」をクラフトします。また任意で「レーザー媒質」もクラフトします
4. 「新品のレーザーブレイドⅠ」または「新品のレーザーブレイドⅡ」をクラフトします
...
金床で新品のレーザーブレイドⅠまたはⅡの名前を`おたから`に変更してから使用すると……
...
- クラフトやレーザーブレイドのアップグレード・修理で入手できるアイテムです
- 手に持って右クリックするとレーザーブレイドを入手できます
...
元のレーザーブレイドの能力値、エンチャント、色などが保存されており、手に持って右クリックするとレーザーブレイドを入手できます。
...
元のレーザーブレイドの能力値、エンチャント、色などが保存されており、手に持って右クリックするとレーザーブレイド（耐火）を入手できます。
...
>古代文明の超技術で作られたかもしれないレーザーの刃を持つ《絶対に刃こぼれしない》剣。レッドストーンで励起させたイオン添加ガラスやグロウストーンの発する光で励起させたダイヤモンドから発生した光を反射によって増幅し、ダイヤモンドのレンズで収束させてからレッドストーンの力場に閉じ込めることで棒状の光の刃にしているという。たぶん光属性なのでアンデッドや邪悪な村人に対して強い。なお本来の用途は切削用の工具であり、古代文明の歴史書によれば、レーザーデバイスの規制が強化される以前は都市部の電気街や観光地の土産物屋でも簡単に入手できたという。

- タイプ：剣
- 基本攻撃速度：2.8
- 基本攻撃力：7
- 耐久値：32000
- 右マウスボタンで盾のようにガード（ブロッキング）できます（デフォルトでは無効。要コンフィグ編集）
...
レーザーブレイドは鍛冶台で特定のアイテムと合成することにより各パーツの色を変更できます。

- 「色付きガラス」と合成すると「刃の外側部分」の色を変更できます  
  ![ ](./img/ver7/color_outer_blade_v7.png "刃の外側部分の色変更")
- 「色付きガラス板」と合成すると「刃の内側部分」の色を変更できます  
  ![ ](./img/ver7/color_inner_blade_v7.png "刃の内側部分の色変更")
- 「カーペット」と合成すると「柄の部分」の色を変更できます  
  ![ ](./img/ver7/color_grip_v7.png "柄の部分の色変更")
- 「遮光ガラス」と合成すると「刃の外側部分」の色を減算モードで描画できます。もう一度合成すると加算モードに戻ります
- 「アメジストブロック」と合成すると「刃の内側部分」の色を減算モードで描画できます。もう一度合成すると加算モードに戻ります

テンプレートにはレーザーブレイドの設計図を使用しますが消費されません。

#### モデルの変更

鍛冶台で特定の素材と合成することにより、そのレーザーブレイドの描画で使用されるモデルを変更できます。

- モデル0：ガラス - デフォルトのモデルです
- モデル1：砂 - 刃が不透明なモデルです
- モデル2：砂岩 - モデル0のバリエーションで、刃の色に減算モードが使用されている場合に見栄えが変わります
- モデル526：土 - モデル0のバリエーションで、鍔がないモデルです

テンプレートにはレーザーブレイドの設計図を使用しますが消費されません。

モデル番号はレーザーブレイドのツールチップで確認できます。

変更されたモデルでの描画はコンフィグファイル`tolaserblade-client.toml`の`useOriginalModelType`と`renderMultipleModels`が両方とも`true`のときにのみ有効です。

#### アップグレード

鍛冶台で特定のアイテムと合成することによりレーザーブレイドを強化できます。
...
鍛冶台で「ネザライトインゴット」を組み合わせると「新品のレーザーブレイド」（耐火）を入手できます。そしてそれを手に持って右クリックすることでレーザーブレイド（耐火）を入手できます。
...
耐久値の減ったレーザーブレイドは鍛冶台で「鉄インゴット」と合成すると「新品のレーザーブレイド」にできます。
...
ディスペンサーにレーザーブレイドを入れてレッドストーン動力を送ると、ディスペンサーの向いている方に対して至近距離の範囲攻撃を行います。
...
バニラのかまどに対してレーザートラップを使用した場合、攻撃する代わりにそのかまどを加熱できます。
...
「分解したレーザーブレイド」は手に持って右クリックすると「LB電池」「レーザー媒質」「レーザーブレイド放射器」「レーザーブレイドの外装」「レーザーブレイドの設計図」を入手できます。そのとき分解したレーザーブレイドに保存されていた能力値、エンチャント、色などは各アイテムへ分割されて保存されます。
...
サーバーで使用される設定です。シングルプレイ時にはクライアント側（統合サーバー）でも使用されます。ゲームフォルダ内のconfigフォルダ内に生成されます。
